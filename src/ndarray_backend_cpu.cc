  ...
  while (indices[0] < shape[0]) {
    ...
    for(int dim_i = dims-1; dim_i >= 0; dim_i--) {
      ...
      if (indices[dim_i] < shape[dim_i]) {
        ...
        indices[dim_i] = 0;
        a_idx -= strides[dim_i]*shape[dim_i];
      }
    }
  }
  /// END SOLUTION
  ...
  while (indices[0] < shape[0]) {
    ...
    for(int dim_i = dims-1; dim_i >= 0; dim_i--) {
      ...
      if (indices[dim_i] < shape[dim_i]) {
        ...
        indices[dim_i] = 0;
        out_idx -= strides[dim_i]*shape[dim_i];
      }
    }
  }
  /// END SOLUTION
...
void EwiseLe(const AlignedArray& a, const AlignedArray& b, AlignedArray* out) {
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = (a.ptr[i] <= b.ptr[i]);
  }
}

void ScalarLe(const AlignedArray& a, scalar_t val, AlignedArray* out) {
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = (a.ptr[i] <= val);
  }
}
...
void EwiseLt(const AlignedArray& a, const AlignedArray& b, AlignedArray* out) {
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = (a.ptr[i] < b.ptr[i]);
  }
}

void ScalarLt(const AlignedArray& a, scalar_t val, AlignedArray* out) {
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = (a.ptr[i] < val);
  }
}
...
void EwiseNeg(const AlignedArray& a, AlignedArray* out) {
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = -a.ptr[i];
  }
}
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  for (size_t i = 0; i < out->size; i++) {
    ...
    while (a_idx < (i+1)*reduce_size) {
      out->ptr[i] = std::max(out->ptr[i], a.ptr[a_idx++]);
    }
  }
  /// END SOLUTION
}
